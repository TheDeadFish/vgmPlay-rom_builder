...
// This code has been stolen and this is probably illegal but I dont care
// Its not like I am going to relese my source code so no one will know
#ifndef _CString_C_
#define _CString_C_
#include <stdio.h>
#include "CString.h"
...
void CString::AllocBuffer(int nLen)
{
	...
	else
	{
		...
		int nSize = sizeof(CStringData) + (nLen+1)*sizeof(TCHAR);
		int nAlloc = 2 << (__builtin_clz(nSize-1)^31);
		CStringData* pData = (CStringData*)new BYTE[nAlloc];
		pData->nAllocLength = ((nAlloc-sizeof(CStringData))/sizeof(TCHAR))-1;
		pData->nRefs = 1;
		pData->data()[nLen] = '\0';
...
int CString::Find(TCHAR ch, int nStart) const
{
	...
	LPTSTR lpsz = _tcschr(m_pchData + nStart, (unsigned)ch);
...
// other stuf not part of the strcore file
...
#endif
#define _countof(x) sizeof(x)/sizeof(TCHAR)
